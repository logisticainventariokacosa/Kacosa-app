// js/control-inventarios.js
// Submódulo "Control de Inventarios" (solo supervisor/coordinador).
// Aquí viven: Apertura/Control (abrir, pausar, reanudar, cerrar) y
// Reportes SAP — antes estaban dentro de "Inventario Físico", que ahora
// es exclusivamente la vista de "todos los conteos" (ver monitor-inventario.js).

(function () {
  let perfilActual = null;

  function iniciar() {
    const link = document.querySelector('.nav a[data-section="control-inventarios"]');
    if (!link) return; // falta agregar el tab en index.html
    link.addEventListener('click', cargarPantalla);
  }

  async function cargarPantalla() {
    const cont = document.getElementById('control-inventarios-contenido');
    if (!cont) return;
    ...
    if (resp.rol === 'analista') {
      cont.innerHTML = '<p class="muted">Este submódulo es solo para supervisores y coordinadores.</p>';
      return;
    }
    perfilActual = resp;
    ...
    cont.innerHTML = resp.inventarios.map(inv => `
      <div class="fisico-inv-card">
        ...
        <div class="fisico-meta muted">
          Apertura: ${window.formatearFechaHora(inv.fecha_apertura)} ·
